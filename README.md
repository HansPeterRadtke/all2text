...
- collect filesystem, MIME, hash, header, classification, converter, warning, and limitation metadata;
- preserve decoded text and structured text exactly in the extracted-content section;
- list archives safely without extracting them;
...
- text, Markdown, source code, JSON, JSONL, CSV, TSV, XML-like text, notebooks, GeoJSON/KML,
  and text-based CAD formats are decoded and preserved in the extracted-content section;
- ZIP/TAR/GZIP archives are listed safely with path traversal warnings;
- EPUB packages are listed and probed as containers;
- EML files are parsed with Python's standard email package, including headers, plain text body,
...
- image, audio, video, PDF, DOC/DOCX, XLS/XLSX, PPT/PPTX, ODT/ODS/ODP, scientific data,
  binary CAD, fonts, executables, disk images, unknown binaries, and specialist containers.
