...
## 2026-06-10 - CAD/BIM and scientific coverage hardening

Direct Jetson implementation, no Codex. Added IFC extension routing to CAD/BIM, text-preservation for IFC, a built-in IFC text schema parser, external IfcOpenShell setup planning, and a subprocess bridge. Jetson installed the IfcOpenShell ARM64 wheel in an isolated Python 3.11 env, but the wheel requires newer glibc than this Jetson provides, so setup now marks it optional and relies on the built-in IFC text schema for required coverage. Scientific coverage now reflects installed numpy, h5py, netCDF4, astropy, pyarrow, and scipy providers. NetCDF schema extraction now falls back to a clean subprocess after an in-process HDF5/netCDF runtime conflict, and successful fallback no longer marks the file warned. Final validation passed: compileall, full pytest, setup dry-run full, doctor full, IFC schema smoke, NPY/NPZ/HDF5/NetCDF/FITS/Parquet/MAT schema smokes, and conversion smoke for IFC/HDF5/NetCDF/Parquet with no errors and no warnings. Full coverage is now 14 covered families, no missing required coverage, and no degraded required coverage.


## 2026-06-10 - Universal rough content labels

Direct Jetson implementation, no Codex. Added explicit rough_content output for image and media files. Images now emit coarse labels such as image_photo_or_scene, image_screenshot_or_ui, image_document_or_scan, image_table_or_spreadsheet, image_chart_or_plot, image_diagram_or_line_drawing, image_logo_or_icon, and image_abstract_or_texture based on deterministic metadata, sampled pixels, structure, and filename hints. Audio/video now emit coarse labels such as audio_silence, audio_speech_or_voice, audio_music_or_stereo_content, video_visual_only_or_silent, video_with_speech_or_voice, video_with_music_or_stereo_audio, and video_with_audio from stream metadata, tags, and WAV waveform stats. Fixed silence detection ordering so short silent WAVs are silence, and made ffprobe visual-only evidence override weak mutagen audio-count noise. Final validation passed: compileall, full pytest, and conversion smoke for chart PNG, screenshot PNG, silent WAV, stereo music-like WAV, and visual-only MP4; all outputs contained their expected rough_content labels.


## 2026-06-11 - Messy all-in-one corpus regression

Direct Jetson implementation, no Codex. Added a committed messy all-in-one corpus under tests/fixtures/messy_all_in_one_corpus with deliberately misleading names/extensions across PNG image, WAV audio, MP4 video, PDF, DOCX, ZIP, GZIP, SQLite, JSON, CSV, GeoJSON, HTML, IFC, DXF, unknown binary, and extensionless text. Added an end-to-end pytest that copies this one folder, injects __pycache__, .pytest_cache, build output, and OS junk, runs all2text once, and asserts classification by content signature rather than filename/extension. The test also checks WAV silence rough-content output and video rough-content output. Scanner now excludes common runtime/build/cache junk by default. Detection now recognizes IFC, STEP, DXF, and ASCII STL from textual content signatures independent of extension. WAV rough-content now infers stream evidence from WAV headers even when optional/external metadata is disabled. Final validation passed: compileall, full pytest, and a rerun of /data/src/github/devtests/rag_tests with cache exclusions; that run produced 97 converted files, 0 errors, no __pycache__ or .pytest_cache entries, and output ratio 0.929189 versus cache-clean source bytes.


## 2026-06-11 - Correct central all-in-one corpus

Corrected the previous over-focused messy fixture. Replaced it with tests/fixtures/all_in_one_corpus, a central regression corpus containing normal/correct files plus wrong-extension variants. It covers text, extensionless text, structured text, source code, email, image, audio, video, PDF, DOCX, XLSX, PPTX, ODT, ZIP, TAR, GZIP, BZIP2, XZ, EPUB, SQLite, NumPy NPY, HDF5, Parquet, FITS, GeoJSON, KML, IFC, DXF, STL, WOFF font, ELF/MZ executables, disk-image extension routing, unknown binary, and nested files. The paired test copies that one folder, injects runtime/build/cache junk, runs all2text once, validates expected family/format/backend for 68 files, proves wrong-extension variants route by content signatures, and checks audio/video rough-content output. Fixed detection precedence so source-code/email/content signatures beat misleading MIME while generic text no longer overrides correct structured-text extensions. Added magic signatures for NPY and font containers. Final validation passed: compileall, full pytest, and external /data/src/github/devtests/rag_tests run with zero errors, no __pycache__/.pytest_cache entries, 97 converted files, and output ratio 0.933391 against cache-clean source bytes.
